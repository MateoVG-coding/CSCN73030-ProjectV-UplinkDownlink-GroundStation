...
public class GroundSender_Exception_ThreadStateException
{
    ...
    public GroundSender_Exception_ThreadStateException(String target, ref Queue<String> payloads, ref Mutex queuelock)
    {
    ...
    public bool SendTransmission(ref String jsonData)
    {
        ...
        if (!transmissionManager.IsAlive)
        {
            if (transmissionStatus)
            {
                try
                {
                    GroundSender_Stubs.StartSendTransmission_Throws_ThreadStateException();
